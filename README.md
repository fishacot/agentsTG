...
[![Status](https://img.shields.io/badge/status-personal%20beta-orange.svg)]()

---

## 📋 Описание

AI Agents Telegram Assistant — мультиагентный «офис» в Telegram: **7 ботов** (оркестратор + 6 ролей). Пишешь нужному коллеге в ЛС или координируешь через Егора. Прод: VPS + Neon, см. [`deploy/FIRSTBYTE_VPS.md`](deploy/FIRSTBYTE_VPS.md).

| Роль (бот) | Сейчас в коде | Planned (не в tools) |
|------------|---------------|----------------------|
| **Эльза** — личный помощник | Задачи (Postgres), заметки Obsidian, `schedule_reminder`, профиль/проект | Календарь, бюджет |
| **Ульяна** — research | `deep_research`, `browser_navigate` / `browser_snapshot` (httpx) | Полноценный browser sandbox |
| **Руслан** — coder | `run_code`, `lint_test` (subprocess) | Docker sandbox |
| **Артём** — security | `scan_prompt` | — |
| **Максим / Дарья** — biz / marketing | `deep_research`, общие memory-tools | CRM, отдельные CRM-tools |
| **Егор** — orchestrator | LangGraph + делегирование, `plan_executor`, JSON v2 supervisor | «Военный совет» как отдельный UI-режим — через `/team` и handoff |

Слои prompt/security (tier LIGHT/STANDARD/FULL, `tool_policy` hook) — см. [`docs/AGENT_PIPELINE.md`](docs/AGENT_PIPELINE.md), [`docs/OPENCLAW_PARITY.md`](docs/OPENCLAW_PARITY.md).
...
│ (tasks, notes, │ │ (deep_research│ │ (scan_prompt│ │ (LLM +   │
│  reminders)    │ │  browser)     │ │  LLM sec)   │ │  memory)  │
└────────────────┘ └───────────────┘ └─────────────┘ └──────────┘
        │
┌───────▼────────┐
│ Marketing      │
...
### Агенты и tools (канон: `services/tools/registry.py`)

| `agent_key` | Telegram | Основные tools |
|-------------|----------|----------------|
| `orchestrator` | Егор | `delegation`, `delegate_task`, `track_progress`, `merge_results` |
| `personal_assistant` | Эльза | `add_task`, `list_tasks`, `create_obsidian_note`, `schedule_reminder` |
| `research` | Ульяна | `deep_research`, `browser_navigate`, `browser_snapshot` |
| `coder` | Руслан | `run_code`, `lint_test` |
| `security_ai` | Артём | `scan_prompt` |
| `business`, `marketing` | Максим, Дарья | `deep_research` + общие `remember_about_user`, `set_active_project`, … |

На всех ролях: tier-правила (привет без tools), gateway hooks (`injection_guard`, `tool_policy`). Приёмка prod: [`docs/E2E_AUTONOMY.md`](docs/E2E_AUTONOMY.md).
...
**Напоминание (Эльза):**
```
👤 Напомни через 3 минуты выпить воды
🤖 Подтверждение с временем МСК → ping в ЛС
```

**Задачи:**
```
👤 Добавь задачу: купить молоко
🤖 Задача в Postgres; список — «покажи задачи»
```

**Ресерч (Ульяна):**
```
👤 Найди свежие новости про Python async
🤖 Ack → финальный ответ отдельным сообщением (background)
```

**Делегирование (Егор):**
```
👤 Составь план: ресерч конкурентов + черновик лендинга
🤖 <b>План:</b> … → шаги специалистам (@username handoff)
```
